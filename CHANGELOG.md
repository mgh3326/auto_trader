...
- **Poll-state tracking.** New `review.toss_fill_poll_state` table records `last_success_at` and `last_error` per scan scope. The discovery service wraps collection+seeding in try/except: on failure it records the error (best-effort) then re-raises so TaskIQ sees the exception.
