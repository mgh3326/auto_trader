...
- **Full and probe smoke lifecycles now prove cleanup from broker evidence.** Bounded open/today-order and position pagination, exact 1-18 ASCII-digit order-ID matching with leading zeroes preserved, baseline position deltas, and bounded cancel polling fail closed on missing, conflicting, filled, unknown, or timed-out evidence. Provider exception text is withheld from operator evidence, registered tools share one OAuth token cache, probes stop immediately after an unsafe baseline, and modify cleanup must prove every known order ID terminal while unknown replacement lineage fails closed.
