...
- 원장이 쓰는 스코프 문자열: `KISLiveOrderLedger.account_mode='kis_live'`(`app/models/review.py:392`),
  `LiveOrderLedger.account_scope ∈ {kis_live, upbit_live}`(`review.py:484`),
  `TossLiveOrderLedger.account_mode='toss_live'`(`review.py:626`).
...
- **감소·해제**: 보호를 푸는 방향 = 전술 매도를 가능하게 만드는 방향이므로 **확인이 더 무겁다**
  ...
  (권고: 허용하되 증거 칸에 `unavailable` 대신 **거부** — 스키마 NOT NULL 유지. 운영자가 브로커 복구 후 다시 한다).
...
| C8 | **레거시 KIS 자동매도 잡** — 보유 전량을 가격 레벨로 균등분할해 직접 발주 | `app/services/kis_trading_service.py:360`(`//` 분할 `:433`), 수량원천 `app/jobs/kis_market_adapters.py:32,44,677,691`, 발주 `:108`/`:129` | 현재 **production 호출자 없음**(`app/jobs/kis_trading.py` 를 import 하는 곳은 AGENTS.md 문서뿐). 구현 PR 에서 삭제 또는 G5 가드 — §7 Q11 | 가드 시: 보호 종목 skip |
...
| T6 | 방어 trim `defensive_trim` · 현금조달 `cash_funding` | 작성자, cash_funding 은 `ceil(required/price)+1` 캡(`cash_funding_exemption.py:285-294`) | 제안 파이프라인만(`orders_registration.py:274-282`, `order_execution.py:1815`) | G1 / G2 | cash_funding 은 자동승인 가능(`auto_approve.py:1012-1039`) |
...
assert_sell_within_protection(
    *, account_scope, market, symbol, quantity: Decimal,
    fresh_broker_sellable: Decimal | None, sellable_observed: bool,
    modify_baseline_quantity: Decimal = 0,
) -> None | ProtectionBlock
```

- 순수 판정 + protected_positions 조회 1회. 판정: `quantity − modify_baseline_quantity ≤ fresh_sellable − P`.
- 행이 없거나 `P = 0` 이면 즉시 통과 — **보호 종목이 아니면 추가 브로커 읽기 0회**(기존 지연·동작 불변).
...
- 차단 결과는 fixed `error_code` 어휘: `protected_quantity_exceeded` · `protected_quantity_encroached` ·
  ...
  `protection_state_unavailable`. 응답에 `protected_quantity`, `broker_sellable`, `headroom` 를 싣는다(비밀 아님).

| 가드 | 위치 (앞/뒤 기준점) | 앞에 두면 안 되는 것 | 신선한 S 의 출처 |
|---|---|---|---|
| **G1** | `_execute_and_record` 안, `side=="sell" and not is_mock` 일 때 **ROB-653 intent reserve(`order_execution.py:1087`) 직전**, 그리고 `_send_to_broker`(`:1178`) 보다 앞 | kis_mock 귀속 게이트(`:973`, 함수 최상단 불변 — 하드룰 7)보다 앞에 두지 않는다. reserve **뒤**에 두면 차단 시 멱등키가 소모되고 `_release_reserved_intent_after_send_failure`(`:1106`)의 "proven absence 만 해제" 규칙과 얽힌다 — 그래서 reserve 직전 | `_get_holdings_for_order` 를 **다시** 호출(C1 의 early 값 재사용 금지 — `:1942` 이후 프리뷰·승인·reserve 사이 시간차) |
| **G2** | `_toss_place_order_impl` 의 `_fresh_sellable_preflight`(`:1746`) 반환 직후, `_toss_pre_send_hook` 조회(`:1756`)·POST 보다 앞 | preflight 보다 앞(fresh 값이 없음) | preflight 의 `fresh_sellable_quantity` 를 그대로 사용 — 브로커 원값이어야 하므로 C6 을 원천에 적용하지 않는 이유 |
| **G3** | `toss_modify_order` 의 fresh 검사(`:2104`) 직후, `client.modify_order`(`:2125`) 앞 | 동일 | 동일. `modify_baseline_quantity` = 원주문 잔량 |
| **G4** | `modify_order_impl` 에서 KR `modify_korea_order` 호출(`:1677`) 앞, US `:1826` 앞, Upbit `cancel_and_reorder` 호출 **앞 = 취소가 나가기 전** | Upbit 에서 취소 뒤에 두면 가드 차단 시 원주문만 사라진다 | `_get_holdings_for_order`(KR 은 이미 `:1289` 에서 읽음 — 재사용 가능, 호출과 송신 사이에 다른 대기 없음을 테스트로 고정). Upbit 은 `S + 원주문 잔량` 을 기준으로 재발주 수량 검사 |
| **G5** | `kis_trading_service._process_sell_orders_impl` 의 각 `ops.place_order` 앞(또는 모듈 삭제) | — | 잡이 이미 읽은 `ord_psbl_qty` |
| **G6 (정적)** | 테스트: 브로커 매도·정정 송신 함수(`order_korea_stock`, `order_overseas_stock`, `modify_korea_order`, `modify_overseas_order`, `place_sell_order`, `place_market_sell_order`, `cancel_and_reorder`, Toss `place_order`/`modify_order`)의 **호출자 allowlist**. 새 호출 지점이 생기면 분류(가드됨/mock 전용) 전까지 RED | — | — |

정정의 가격만 변경(`new_quantity` 없음 또는 ≤ 원잔량)은 보호를 악화시키지 않으므로 `encroached/shortfall`
에서도 허용한다(Δ ≤ 0). 증량만 headroom 으로 검사한다.
...
| KIS `ord_psbl_qty` 가 비어 총보유로 폴백됨(`sellable_observed=False`, `order_validation.py:821`) | 거부 — 폴백 값은 미체결 매도를 모른다 |
...
- 모드 배너: `PROTECTED_QUANTITY_MODE` 가 `off`/`shadow` 면 화면 상단에 **"보호 미강제 — 표시·기록만"** 을
...
| R1 | 서비스·읽기 표면(C5·C7 필드 추가, 단 **값 치환은 아직 안 함**)·설정 화면 쓰기. 운영자가 보호를 입력 | `PROTECTED_QUANTITY_MODE=off`(기본) |
| R2 | shadow: G1–G5 판정 + `would_block` 기록, L1 치환 없음. 최소 N 거래일(권고 10) 관측 — 오탐(`unverified` 빈도)·누락 확인 | `=shadow` |
| R3 | enforce: L1 치환 + G 차단 | `=enforce` — 운영자 명시 전환 |
...
- 1차: `PROTECTED_QUANTITY_MODE` 를 `shadow`/`off` 로 되돌리고 재시작. 데이터(선언·이력)는 남는다.
...
| Q13 | Toss `sellableQuantity` 가 미체결 매도를 빼는지 | 구현 PR 에서 실모양 fixture 로 **확정 전 enforce 금지**(Toss 스코프만 shadow 유지) |
