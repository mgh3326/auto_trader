...
| 5 | 돈 잘버는 회사 찾기 | 최근 1년 TTM 매출총이익률 ≥20% + ROE ≥15% | — | **missing** | `roe`✅ / `gross_margin_ttm`❌ | roe: valuation / 매출총이익률: unavailable | ❌ no (source gap) |
...
| 10 | 고수익 저평가 | ROE ≥15% + PER 0~10 | — | **missing (즉시 구현 가능)** | `roe`✅ `per`✅ (둘 다 보유) | valuation (둘 다) | ✅ yes (feasible now) |
...
| 상태 | 개수 | 프리셋 |
|---|---|---|
| **full** | 2 | 연속 상승세(`consecutive_gainers`), 쌍끌이 매수(`double_buy`) |
| **partial** | 2 | 아직 저렴한 가치주(`cheap_value`), 꾸준한 배당주(`steady_dividend`) |
| **mismatch** | 2 | 저평가 탈출(`oversold_recovery`), 성장 기대주(`growth_expectation`) |
| **missing** | 5 | 저평가 성장주, 돈 잘버는 회사 찾기, 미래의 배당왕 찾기, 고수익 저평가, 안정 성장주 |
| **extra** | 2 | 거래량 급증, 수급 모멘텀 |

- 2개(full) = ROB-170/276에서 이미 Toss parity 구현 완료. **재조사 불필요.**
- 4개(partial/mismatch)는 Acceptance criteria 대상: 의미 정정 또는 `partial/mismatch` 명시 필요(PR3 Scope B).
- 5개(missing) 중 **고수익 저평가만 기존 `market_valuation_snapshots`(roe+per)로 즉시 구현 가능**.
...
- **고수익 저평가** (ROE≥15 + PER 0~10) — `market_valuation_snapshots` join만으로 신규 preset 추가 가능.
