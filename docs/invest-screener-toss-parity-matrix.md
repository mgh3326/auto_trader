...
| `support_proximity` | 지지선 근접 | 시총 ≥3천억 + 거래대금 ≥10억 + `dist_to_support_pct` 오름차순 | **extra** | auto_trader 자체 스크린 (ROB-976). bounded 빌더가 `get_support_resistance`(fib/거래량프로파일/볼린저)를 완료봉에 적용해 가격·지지선·거리를 함께 저장하며, 조회는 저장분만 반환. Toss 기본 프리셋 아님. |
...
| **mismatch** | 0 | — |
| **missing** | 0 | — |
| **extra** | 5 | 거래량 급증, 수급 모멘텀, 과매도 반등 (RSI), 대형 모멘텀 (시총·등락률), 지지선 근접 |
