...
| 상태 | 개수 | 프리셋 |
|---|---|---|
| **full** | 7 | 연속 상승세(`consecutive_gainers`), 쌍끌이 매수(`double_buy`), 고수익 저평가(`high_yield_value`), 돈 잘버는 회사(`profitable_company`), 저평가 성장주(`undervalued_growth`), 안정 성장주(`stable_growth`), 미래의 배당왕(`future_dividend_king`) |
...
- 7개(full): 연속 상승세/쌍끌이 매수 = ROB-170/276, 고수익 저평가 = ROB-359, 돈 잘버는 회사 = ROB-422 PR2a, 저평가 성장주/안정 성장주/미래의 배당왕 = ROB-422 PR2b.
