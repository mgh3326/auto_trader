...
| `support_proximity` | 지지선 근접 | 시총 ≥3천억 + 거래대금 ≥10억 + `dist_to_support_pct` 오름차순 | **extra** | auto_trader 자체 스크린 (ROB-976). `get_support_resistance`(fib/거래량프로파일/볼린저) 재사용, 상위 후보만 실시간 재검증. Toss 기본 프리셋 아님. |
