...
- **Double gate (hard rule 4)**: `NHPLUG_MOCK_ENABLED` (default false) is re-read from the process environment at every dispatch; `settings.nhplug_mock_enabled` only controls DEFAULT-profile MCP registration. Every mutation needs `dry_run=False` **and** `confirm=True`: the MCP schema uses `StrictBool`/`StrictInt` (JSON `0`/`1`/`"true"` are rejected at validation, never coerced); the tool/operations layer checks identity with the bool singletons; the client checks again at each public order method **and** inside the `_post_mutation` dispatcher (exact type `DryRunConfirmContract`, no subclass, fields compared by identity) before any token or socket I/O. `dry_run=True` is fully offline (no network, no ledger row).
- **Post-build re-verification**: for reads and orders the built request's scheme/host/port/path is rechecked immediately before `send`; for account-scoped reads and every order the `act_no` is decoded from the exact request bytes and rechecked against the broker-derived `acct_type=03` allowlist. Orders additionally require the allowlist this **client instance** derived from its own `/n2/acctinfo` request (`verify_and_bind_mock_account`, fresh per operation); a caller-supplied allowlist (`bind_account_allowlist`) enables reads only, and `MockAccountAllowlist` cannot be constructed except by `from_acctinfo_response`. `follow_redirects=False` on every client; a 3xx on an order is dispatch-uncertain, never followed.
...
- 🔴 **Empty array ≠ no open orders** (Kiwoom `kt00009` lesson): the vendor omits response blocks when empty and one `rsp_cd` means different things per API. A listing is usable only with a known read code, a list row block (or none), fully parsed rows, and completed pagination: a page is final only with no continuation key (header or body), no continue code, and no `cts_flag=Y` — any key is followed even if `cts_flag=N`, and an unfollowable signal or a repeated key leaves the listing incomplete. **There is no "none" answer**: `open_orders_state` is `present` (an open row positively listed by a complete scope) or `unknown`; an empty, block-absent, `13578`, or error-shaped open-only response is always `unknown` (`empty_open_listing_is_not_evidence_of_no_open_orders`, `success=false`). **Absence is never evidence**: the open-only scope is used only to detect disagreement. Terminal states need two positive sources — filled: the all-orders row and the filled-only scope (`ost_cns_dit=1`, same quantity); cancelled: the all-orders row and our own broker-acknowledged cancel for that order (`ack_order_id`, set only by `record_ack`); modified: the all-orders row, our own acknowledged modify, and the successor row naming the original; `confirmed` (a cancel row): its own ack and the original's cancelled quantity; a listing-derived rejection goes to manual review. Status derivation requires filled + open + cancelled + modified to equal the order quantity exactly. The ledger writer refuses any quantity/evidence field unless `reconcile_state='verified'`; table CHECKs `filled_qty <= quantity` and `ack_order_id = broker_order_id`.
- **No ledger row, no send; exactly one send of exactly the committed body — enforced by the database**: the client's only order method, `dispatch_claimed_order(ledger, ledger_row_id, client_request_id, expected, authorization)`, accepts no quantity, price, or order object. Immediately before `send` it calls `NHPlugMockLedgerService.claim_for_dispatch` (exact type required): one atomic conditional `UPDATE` moves the row `submitting -> dispatching` and sets a unique `claim_token`/`claimed_at` only `WHERE` the id, `client_request_id`, `status='submitting'`, `claim_token IS NULL`, and the exact operation/symbol/side/quantity/price/original all match, committed before any byte is sent. The request body is built only from the values that `UPDATE ... RETURNING` produced, and the built bytes are re-checked against them. A replay (same or new request id), a second client or process, a concurrent call, or a drifted expectation matches no row (`NHPlugMockClaimRejected`, nothing sent, row untouched). DB CHECKs: `submitting` ⇒ unclaimed, every state other than `submitting`/`not_submitted` ⇒ claimed, claim token/time paired and unique. An unclaimed `submitting` row provably never reached send, so reconcile treats `dispatching` (not `submitting`) as "may have been sent".
