...
- **No ledger row, no send — at the client**: every client order method and `_post_mutation` require a single-use `CommittedOrderIntent` that only `NHPlugMockLedgerService.committed_intent` issues, after re-reading the committed `submitting` row; the intent must match the order body (checked before token I/O and again on the built request bytes) and is consumed before `send`. A static guard allows `issue_committed_intent` only in the ledger service.
