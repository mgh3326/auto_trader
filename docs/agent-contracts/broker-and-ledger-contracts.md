...
- 🔴 **Empty array ≠ no open orders** (Kiwoom `kt00009` lesson): the vendor omits response blocks when empty and one `rsp_cd` means different things per API. A listing is usable only with a known read code, a list row block (or none), fully parsed rows, and completed pagination: any continuation signal (`cts_flag=Y`, a continue code, a body key) that cannot be followed, or a repeated key, leaves it incomplete. `open_orders_state="none_confirmed"` requires the open-only scope (`ost_cns_dit=2`) and the all-orders scope (`ost_cns_dit=0`) to be complete and agree, a **liveness witness** (the all-orders listing returns at least one of today's rows — an all-empty day is `unknown` with `empty_listing_is_not_evidence_of_no_open_orders`), and no live/uncertain ledger order missing from the listing; otherwise `unknown` (`success=false`). Status derivation requires filled + open + cancelled + modified to equal the order quantity exactly; any non-zero fill must also appear with the same quantity in the filled-only scope (`ost_cns_dit=1`). The ledger writer refuses any quantity/evidence field unless `reconcile_state='verified'`, and the table CHECKs `filled_qty <= quantity`. Reconcile never closes a row from an incomplete listing or from absence: terminal needs the all-orders row to say so **and** absence from a complete open-only scope; disagreement is `source_disagreement`, missing is `unknown`.
