# ROB-512 갭3 — KR/US 한글 카테고리: 마스터 테이블 + lazy fill (스펙)
...
5. **US도 동일 구조 적용** (2026-06-11 추가 결정): `us_symbol_universe`에 같은 컬럼 추가 + lazy fill. 소스는 yfinance `info`(영문 industry/sector) → **정적 한글 세분류 매핑**(industry ~145종, "Semiconductors"→"반도체") 적용 후 저장, 매핑 미스는 영문 원문 저장(fallback honest).
...
`kr_symbol_universe`와 `us_symbol_universe` **둘 다**에 nullable 컬럼 2개 (단일 migration):

| 컬럼 | 타입 | 의미 |
|---|---|---|
| `sector` | String(100) nullable | 표시용 카테고리 — KR: Naver 한글 업종명 그대로 / US: 한글 매핑 적용값(미스는 영문 원문) |
| `sector_updated_at` | TIMESTAMP(tz) nullable | 채운 시점 (이번엔 기록만; stale 갱신 정책은 향후) |

실 DB 확인(2026-06-11): 두 테이블 모두 sector류 컬럼 없음. kr active 3,914행 — migration 직후 전부 NULL에서 lazy 워밍업 시작.

**sync 보존 (검증됨)**: `kr_symbol_universe_service._apply_snapshot`(:240-296)은 name/exchange/nxt_eligible/is_active만 필드 단위로 갱신 → sector는 sync에 지워지지 않음. 회귀 테스트로 고정한다. **US sync도 동일 방식인지 plan 단계에서 확인 후 같은 회귀 테스트 추가** (필드 단위가 아니면 sector 보존 로직 보강).
...
**US (market == "us")**: 같은 단계에서 미러로 동작.
1. `us_symbol_universe.sector` 일괄 조회 → NULL 심볼만 yfinance `Ticker.info`에서 `industry`(우선)/`sector`(fallback) 추출 (DB 심볼→yahoo 심볼은 기존 `to_yahoo_symbol` 변환, sem 4, timeout, fail-open)
2. 정적 한글 매핑 모듈(`industry` ~145종 + `sector` 11종, 예: "Semiconductors"→"반도체") 적용 — **매핑 미스는 영문 원문 저장**(빈 값/fake 금지)
3. persist + 이번 응답 category 교체 (KR과 동일)
...
- **US 로더**: market=="us" 경로의 결과 row에 `us_symbol_universe.sector` lookup 추가 (해당 시장 로더들의 기존 universe 조회에 합류; 구체 배선 지점은 plan 단계에서 US 로더별 확인).
...
| US 매핑 | 매핑 존재 industry→한글, 미스→영문 원문 저장; lazy fill 미러(fake fetch) |
...
- crypto 카테고리. (US는 2026-06-11 결정으로 스코프 **포함**)
