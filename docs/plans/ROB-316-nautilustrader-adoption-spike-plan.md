...
## 15. Out-of-sample 확인 (~60일) — §14의 session edge는 과적합이었다

§14의 session-only 양수는 under-powered(25 trades)로 flag됐다. 표본을 **~60일**(2026-03-26~05-24,
~6.3M ticks; breakout 100/100 = **232 trades**, session-only = **111 trades** — 둘 다 30 초과)로 키워
out-of-sample 재검증. (catalog는 60일로 재적재; 동일 harness.)

**결과 (NET PnL USDT / win% / GROSS bps):**
```
  strategy            trades   net@10  net@7.5   GROSS   win%
  breakout 100/100       232    -76.5   -60.4   -11.9   48.7
  ict session-only       111    -47.2   -39.5   -16.3   45.0
```

**14일 결과를 뒤집음:**
- breakout 100/100 GROSS: 14일 **+9.4 → 60일 −11.9** (부호 반전). 14일의 gross edge는 **노이즈**였다.
- session-only: 14일 gross +6.9 / win 60% → 60일 **gross −16.3 / win 45%**. **session 필터는 도움 안 됨**
  — 더 큰 표본에서 오히려 더 나쁨.
- 둘 다 realistic fee에서 크게 손실.

**결론:**
- §14의 "session 필터가 edge를 올린다 / 7.5bps net 양수"는 **14일 윈도우 과적합 artifact**. overfit 경고(25 trades)가 정확했다.
- 통계적으로 의미 있는 표본에서 **breakout(100/100)도 session-filtered도 durable edge 없음.**
  이 신호 계열(trend micro-breakout)은 fee/타깃/시간필터 어떤 조합으로도 살아나지 않는다.
- **L2/실시간 paper 확장 미진입 확정** — 신호 자체에 edge가 없으므로 인프라 확장은 무의미.
  필요한 건 튜닝이 아니라 **근본적으로 다른 alpha**.

**방법론적 수확:** overfit flag → 표본 확대 → out-of-sample 검증 파이프라인이 작동함을 입증.
앞으로 어떤 신호든 이 절차(작은 표본 결과는 OOS로 반드시 재검증)로 판단한다.

**검증:** `compare_strategies.py --catalog catalog --specs "breakout 100/100,ict session-only"`
(export: `results/compare_largesample.csv`). No broker/order/secret side effect; 공개 데이터만.
