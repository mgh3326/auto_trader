...
**Design revision (supersedes Task 6 backfill, resolves B2 by elimination):** The probe CLI is **strictly read-only — no `--backfill-window` / `--confirm` / write surface at all.** Materializing the dev-DB window reuses the established, tested `scripts/backfill_daily_candles.py --market us --symbols … --horizon-bars …` (which builds the sync service via `_build_default_service`); the runbook documents the backfill→re-probe procedure (dev DB only). This is the clean cut: ROB-371's new code carries zero mutation path, so the B2 prod-DB-guard concern is moot rather than guarded. The gate's not-materialized verdict already points operators at this step. A read-only probe reading prod is acceptable (cf. `diagnose_calendar_coverage.py` "safe to run against production"), so no DATABASE_URL guard is needed. B3 counts-only enforcement still applies: `build_artifact` is asserted scalar-only.
