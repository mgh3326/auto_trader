...
- 동작(행 단위, 순서 보존): 행 writer 종류는 기존 `order_proposal_create`(기존 가드·approval_hash·clientOrderId 규칙 그대로), `investment_watch_create`, `forecast_save` 세 가지다. 현재 비준된 매핑으로 집행하는 행은 proposal 또는 watch이고, forecast는 아래의 미비준 mapping이 해소될 때까지 fail-closed한다. `session_context_append`는 행 종류가 아니라 **apply 1회당** 모든 행 처리가 끝난 뒤 남기는 요약 1건이다. 제안 rungs는 v1.1 `{rung,price_min,price_max,qty,tick}`에서 proposal `{rung_index,side,quantity,limit_price,notional}`으로 명시 매핑하며, pinned 가격만 `limit_price`로 쓴다. 직접 브로커 호출은 없고 주문 전송은 기존 승인 경로에만 남는다.
...
- forecast 보류: `forecast_save`는 `instrument_type`, typed `forecast_target`, `probability`를 요구하지만 위 v1.1 additive forecast 행에는 정본 target/probability 매핑이 없다. 이 mapping이 비준되기 전까지 forecast 행은 `invalid_row_mapping`으로 fail-closed하고 `forecast_save`를 호출하지 않는다. 따라서 현재 실제 집행 범위는 proposal/watch 행뿐이다.
