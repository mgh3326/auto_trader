...
| 마켓 범위 | **전 라이브 마켓** — US/crypto(`LiveOrderLedger`) + KR(`KISLiveOrderLedger`) 통합. mock/paper/alpaca/binance/toss는 `report_item_uuid` 없거나 별도 → 범위 밖 |
...
- fills 페이지(SellHistoryPanel), intraday ActionPacketView, mock/paper/alpaca/binance/toss ledger는 범위 밖.
