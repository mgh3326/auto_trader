...
3. mock_preview runner: account_scope=kis_mock / execution_mode=mock_preview
   (live item projection + cited_snapshot_uuids 보존; mock 번들은 production
   collector registry로 account-bound + account-독립 evidence를 **별도 수집**).
4. mock preview 브리지: KIS-mock 단독 read-only preflight, submit OFF.

> **알려진 한계 (ROB-379 smoke로 증명됨):** 현재 구현은 공통 evidence를 "1회 수집해
> 재사용"하지 **않는다**. live ensure와 mock ensure가 각각 account-독립 collector를
> 재실행하는데, market/news/symbol 등은 시변(time-varying) payload라 수초 간격에도
> canonical_payload_hash가 달라져(예: live market `2a4be2d9` vs mock `67fb9dbf`,
> live news `0c06d59e` vs mock `4acfbb4e`) dedup이 미스나고 **별도 NULL-scope row**가
> 생긴다 → 두 리포트가 **공유 snapshot 0건**. NULL-scope 정규화는 필요조건일 뿐
> 충분조건이 아니다. 진짜 "1회 수집 + 재사용"은 mock 경로가 live 번들의 account-독립
> snapshot을 **명시적으로 재사용**(live bundle_uuid 주입 / 공유 수집 패스)해야 하며,
> 이는 별도 follow-up이다.
