...
> **관련 runbook:** 체결(fill) 이벤트 트리아지는 [`docs/runbooks/fill-event-claude-triage.md`](fill-event-claude-triage.md) (ROB-755)를 참조.
...
~/ops/watch-alert-triage/poller.sh   (레포밖 운영자-호스트)
         ...
         │
         ├─ DRY_RUN=1 → 명령 출력만 (claude/Discord/Telegram 미호출)
         │
         └─ DRY_RUN=0 →
...
echo "$events" | jq -c '.[]' | while read -r ev; do
  ...
  if [[ "$DRY_RUN" == "1" ]]; then
    ...
    # Discord 실패 후 재시도 시 동일 uuid가 중복 기록될 수 있음 — 허용(집계 시 dedup 가능).
...
<plist version="1.0"><dict>
  ...
  <dict>
    ...
    <key>TELEGRAM_CHAT_IDS_STR</key><string>123456789</string>
  </dict>
...
**목적:** 워터마크/디듀프 로직과 claude 호출 명령 조립이 올바른지 확인 (실 claude/Discord/Telegram 호출 없음).

```bash
mkdir -p ~/.local/state/watch-alert-triage
DRY_RUN=1 AUTO_TRADER_REPO="$AUTO_TRADER_REPO" \
...
**전제:** Step 4 통과 후. market에 맞는 `DISCORD_WEBHOOK_*`와 `TELEGRAM_TOKEN`/chat ID 설정 필요.

> **경고:** 워터마크를 리셋하면 `--limit 50` 범위의 모든 이벤트가 한 번에 재처리된다(각각 claude 비용 + Discord 발송). 먼저 Step 2로 count를 확인하고 재처리 건수를 파악하라.
...
DRY_RUN=0 AUTO_TRADER_REPO="$AUTO_TRADER_REPO" \
  bash ~/ops/watch-alert-triage/poller.sh
```
...
```bash
export DISCORD_WEBHOOK_CRYPTO="https://discord.com/api/webhooks/..."
export TELEGRAM_TOKEN="..."
export TELEGRAM_CHAT_IDS_STR="123456789"
```

market이 `kr` 또는 `us`이면 `DISCORD_WEBHOOK_KR` 또는 `DISCORD_WEBHOOK_US`를 설정한다. launchd 실행 시에는 plist의 `EnvironmentVariables`에 같은 값을 넣는다.
...
Discord 전송 실패 시 해당 이벤트는 seen-set에 기록되지 않아 **다음 사이클에 재트리아지·재전송된다(at-least-once)**. `validation.jsonl` 에는 claude 성공 시점에 이미 기록되어 있으므로 재시도 시 동일 uuid가 중복 기록될 수 있다(집계 시 허용 가능). Discord webhook이 영구적으로 잘못 설정된 경우 매 사이클마다 claude 재호출 비용이 발생하므로 `stderr.log` 의 `discord post 실패(재시도 예정)` 메시지를 지속 모니터링하라. webhook URL과 채널 권한을 확인.
