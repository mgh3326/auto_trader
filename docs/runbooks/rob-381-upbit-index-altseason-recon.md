...
```jsonc
{
  ...
  "altseason": {
    ...
    "krw_alts_beating_btc_24h_pct": 0.42, // PR2 기본: official /v1/ticker 24h 자체계산
    "breadth_window": "24h",              // PR2=24h만. 7/30/90d는 PR2b(/v1/candles/days)
    "method": "open_api_ticker_24h_derived"
  }
}
```

> **breadth 계약 주의**: official `/v1/ticker`는 **24h 변화율(`signed_change_rate`)만** 제공하고
> 7/30/90d 다기간 변화율은 주지 않는다(다기간은 robots-disallowed `crix-api-cdn`의
> `interval_change_rate`에만 있음). 따라서 PR2 기본 breadth는 **24h alt-vs-BTC breadth**로
> 한정하고, 7/30/90d breadth는 official `/v1/candles/days` fanout(심볼별 호출 → 캐시/레이트리밋
> 설계 필요)으로 별도 **PR2b/후속**에서 다룬다.
...
> **fixture 주의**: `crix-api-cdn`/`datalab-static` 샘플은 **관측된 스키마를 기록**하기 위한 것이지
> production 입력 계약이 아니다. PR2 production 코드는 robots-disallowed `crix` 응답이 아니라
> **official Open API `/v1/ticker`(및 PR2b의 `/v1/candles/days`) 응답 전용 normalizer**를 구현해야 한다.
> 두 응답의 필드/형태는 서로 다르므로 crix fixture를 production 파서에 재사용하지 않는다.
...
| 데이터 | PR2 소스 | 근거 |
|---|---|---|
| 디지털자산 지수 / UBAI·UBMI / 요약통계 / 알트시즌 ratio | **`datalab-static.upbit.com`** (PR2) | robots 제약 없음(S3 공개 데이터 제품), 캐시 가능, 안정적 |
| **24h** breadth(알트>BTC 비율) | **공식 Open API `api.upbit.com/v1/ticker`로 자체 계산** (PR2) | robots-disallowed `crix-api-cdn` 회피, 이미 보유한 클라이언트. `/v1/ticker`는 24h만 제공 |
| **7/30/90d** breadth(다기간 상승률) | **공식 `api.upbit.com/v1/candles/days` fanout** (PR2b/후속) | `/v1/ticker`엔 다기간 없음 → 심볼별 일봉 fanout + 캐시/레이트리밋 설계 필요한 별도 PR |
| 체결강도(`daily_volume_power`) | **park** | `crix-api-cdn` robots Disallow, 부가가치 낮음 → 보류 |

## Recommendation (다음 단계, PR1에서는 구현 안 함)

1. **PR2**: `datalab-static` 지수 + 공식 `/v1/ticker` **24h** breadth 자체계산으로 `get_upbit_index` /
   `get_upbit_altseason` MCP read tool 구현 (ROB-377 4-layer fail-open + in-memory 캐시). PR2 production
   코드는 crix fixture를 재사용하지 않고 **official ticker 응답 전용 normalizer**를 구현한다(아래 fixture 주의 참고).
2. **PR2b/후속**: 7/30/90d breadth는 official `/v1/candles/days` 심볼별 fanout + 캐시/레이트리밋 설계로 별도 진행.
3. **PR3(선택)**: `UpbitIndexSnapshotCollector`를 `production_collector_registry`에 등록 → Hermes crypto market dim 보강.
4. 모든 호출에 식별 가능한 User-Agent + 보수적 폴링(≥60s) + 장기 캐시. robots-disallowed 호스트(`crix-api-cdn`)는 production 통합에 사용하지 않음.
