...
| `inquire_overseas_buyable_amount` | cash/orderable | `/uapi/overseas-stock/v1/trading/inquire-psamount` | `TTTS3007R` | `VTTS3007R` — **called** via the mock-only `inquire_mock_overseas_buyable_amount()` (ROB-951; verified on the mock host 2026-07-17). Sole USD orderable-cash source for mock US, replacing the OPSQ0002 `inquire_overseas_margin` path | mock TR only (no live mode); `ord_psbl_frcr_amt` = verified USD orderable cash. Fail-closed: non-zero `rt_cd` / missing output raises, callers keep `None` (never 0) | — | `tests/test_kis_account_fetch_stocks.py::test_inquire_mock_overseas_buyable_amount_uses_vtts3007_and_parses_usd`, `tests/test_order_us_mock_buy_unsupported.py`, `tests/services/us_dual_paper/test_kis_mock_adapter.py` |
