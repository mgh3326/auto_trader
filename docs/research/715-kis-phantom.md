...
3. **KR evidence-window cap.** TTTC8001R is probed on the exact order date with a 90-day cap; older rows probe a wrong date and almost always match nothing → FillVerdict.NONE → noop_no_evidence + requires_manual_review forever.
4. **Coverage/invocation gap.** All reconcile entry points are off by default or operator-invoked, and every default call uses limit=100 created_at-ASC — permanently unresolvable oldest rows (gaps 1, 2, 3 all leave rows in the open scan) starve every scan, so even an armed reconcile would never reach newer resolvable rows. (The KR terminal-repair pre-pass has the same starvation pattern: LIMIT applied in SQL before the rung-state filter — kis_live_order_ledger_service.py:59,63-68.)
...
- Verdict: resolvable without guessing for **zero-fill** rows within the ~90-day inquiry depth (comment at kis_live_ledger.py:385-387). **Partial-residual expiry evidence is NOT verified anywhere**: classify_day_order_expiry returns expired only on rjct_qty==ord_qty>0, and tests/services/brokers/kis/test_live_order_expiry.py:127-132 pins ord_qty=10/rjct_qty=6/rmn_qty=0 → pending after NXT close. No live-verified TTTC8001R shape exists for a partially-filled-then-swept order — a probe is required before any classifier rule is written (see Fix B). For >90 days: tester flagged that KIS publicly documents a separate 3-months-ago daily-order TR (CTSC9115R/CTSC9215R) — not in the repo, unverified; treat "unrecoverable" as a question for operator-desk, not a fact.
...
- The KR/US split is NOT in the masked extract, but it IS in the raw sweep.json on NCP: rows[].market and rows[].ledger_rows[].{table,status,reconciled_at} are in the payload (resting_sweep.py:170-201). One jq on /root/at-run/sweep715/sweep.json answers the split and the accepted/partial mix (tests the already-booked half of gap #1) — no new query, no broker call. Ages are NOT in sweep.json (as_row carries only observed_at; census.truncation_proof_by_window is whole-population across all account modes with floors ≥55 days back) — ages need the DB-side join in Q6.
...
- toss_live 12 no_ledger_row (INFERENCE): all 3 samples show rung_state=unverified with blank broker_order — so the surviving match key is idempotency_key (client_order_id) or correlation_id, not broker_order_id, and no row matched in any of the three ledgers (resting_sweep_service.py:50-69, 117-161). The most direct reading is an ambiguous submit: an accepted Toss response commits its ledger row before the rung can transition (service.py:1537-1541), so a submit that raised or timed out leaves no ledger row. Resolution path already exists: order_proposal_void(proposal_id, reason) + fetch_operator_void_evidence scans Toss OPEN + bounded CLOSED windows (keys read server-side from the rung, with a symbol/side/qty/price/time fallback match — broker_gateway.py:285+, :518-601). What's needed to run it: the proposal ids, a void authority (authorize_void allows only self_created / server_loss_guard_invalid / server_expired — order_proposal_tools.py:845-860, service.py:1508-1516), and the elapsed unverified settlement grace (service.py:1543-1556).
...
Step 0 — free, no broker calls, answers the KR/US split and partial/accepted mix:
    ...
    jq on /root/at-run/sweep715/sweep.json: group rows[] by account_mode+market+ledger_rows[].table+ledger_rows[].status.

Step 1 — KR leg (writes nothing; read-only TTTC8001R GETs; CLI has no --limit flag so use the kernel for full coverage; LOAD WARNING: limit=800 means up to 800 paginated broker calls — start small):

    uv run python -m scripts.kis_live_auto_reconcile            # dry-run, limit=100
    uv run python -c "import asyncio,json; from app.mcp_server.tooling.kis_live_ledger import kis_live_reconcile_orders_impl; print(json.dumps(asyncio.run(kis_live_reconcile_orders_impl(dry_run=True, limit=50)), ensure_ascii=False, default=str))"

    Look for: counts{expired,cancelled,filled,partial,pending,none}; reconciled[].action — would_mark_expired / would_mark_cancelled / would_book_filled = resolvable today (KR would_mark_* still needs a second non-dry pass or sweep --apply to converge the rung). **verdict=partial with would_book_partial or noop_already_booked is the gap-#1 signature, NOT resolvable** — under current code a non-dry pass books it to ledger partial and strands the rung resting forever, so do not run a pre-fix non-dry backfill over these. noop_no_evidence + requires_manual_review = permanent-skip (>90d or unmatched). candidate_scan{scanned,open_total,limit} for truncation (705 backlog > any small limit); proposal_rung_sweep.summary stays NO_EVIDENCE in dry-run.
...
Step 2 — US leg (read-only TTTS3035R GETs; WARNING on load: each row re-scans the full 7-day all-symbol history across up to 4 exchanges with pagination — with limit=800 this can mean thousands of KIS calls; prefer a small limit first, and use market/broker filters):
    ...
    Look for: counts{expired,cancelled,filled,partial,pending}; the US kernel emits would_book (not would_book_filled). CRITICAL: noop_pending rows do NOT carry reason_code — not_found is invisible in this output (tester B1), and Step 0's ledger status/reconciled_at cannot separate them either (noop writes nothing, so aged-out and live rows look identical). The only discriminators are the DB age join (reconciled[].ledger_id → created_at/trade_date >7 days) and the Step 3 probe.
