    ...
    try:
        ...
        for stock in kr_stocks:
            ...
            try:
                ...
                print(
                    f"      보유: {quantity_num}주 | 평균매입: {avg_price_num:,.0f}원 | 현재가: {current_price_num:,.0f}원"
                )
                print(
                    f"      평가금액: {eval_amount_num:,.0f}원 | 손익: {profit_loss_num:,.0f}원 ({profit_rate}%)"
                )
            except (ValueError, TypeError) as e:
                # 변환 실패 시 원본 값 그대로 출력
                print(f"  - {stock_name} ({stock_code})")
                print(
                    f"      보유: {quantity}주 | 평균매입: {avg_price}원 | 현재가: {current_price}원"
                )
                print(
                    f"      평가금액: {eval_amount}원 | 손익: {profit_loss}원 ({profit_rate}%)"
                )
                print(f"      (포맷팅 실패: {e})")
