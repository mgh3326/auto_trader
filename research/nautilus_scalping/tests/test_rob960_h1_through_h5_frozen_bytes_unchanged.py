...
ROB-962 authorized the H4-bound repair at ``237cd38a`` (the exact-float
``0.0`` cost-model seed) as the new byte authority and retained this guard
class.  Only the two guards' self-referential re-pin metadata are excluded;
every other frozen predecessor byte remains protected.
"""
...
_FROZEN_BYTE_AUTHORITY = "237cd38a3f9e584ddb1071c74d3d442b53f6bd1f"
...
def test_h1_through_h6_and_h5_owned_paths_are_byte_identical_to_the_required_starting_head():
    ...
    assert changed == [], (
        "after the authorized ROB-962 H4-bound repair, no later change may "
...
def test_the_diff_filter_is_not_vacuous_newer_lineage_files_would_otherwise_show_up():
    ...
    paths must be non-empty (it includes later-lineage Added support files),
    so the filtered assertion above is a genuine exclusion, not a
    vacuously-always-passing check."""
    unfiltered = _git_diff_names(diff_filter=None)
    assert unfiltered, (
        "expected the unfiltered diff to be non-empty (newer-lineage files "
        "exist) -- if this is empty, the filtered test above proves nothing"
    )
