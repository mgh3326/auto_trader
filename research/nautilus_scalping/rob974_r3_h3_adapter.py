...
__all__ = ["evaluate_r3_s3_gates", "evaluate_r3_s4_gates"]
...
def evaluate_r3_s3_gates(
    metrics: s3.S3Metrics, config: R3S3Config
) -> s3.S3GateOutcome:
...
def _s4_risk_distances(
    config: R3S4Config, sigma_pair: float
) -> tuple[float, float]:
...
def evaluate_r3_s4_gates(
    ...
    if not predicates.s4_prior_z_magnitude_passes(
        estimate.z_prior, config.z_entry
    ):
    ...
    if not predicates.s4_absolute_distance_passes(
        estimate.D_bps, config.d_min_bp
    ):
