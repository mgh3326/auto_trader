...
## 🔴 v2 — r2 수리 라운드 (2026-08-21 적대검증 반려 반영)

1차 보고는 T2 적대검증에서 **BLOCKER 2 / SHOULD 5 · VERIFIED=NO** 로 반려됐다
(원문 `~/work/herdr-inbox/answer-gatebtv-20260821.md`). 이 문서는 그 수리본이며,
지적을 요약이 아니라 **그대로** 싣는다.

| ID | 지적 | 수리 | 위치 |
|---|---|---|---|
| **B1** | 러너 `[:pos+1]`→`[:pos+2]` 뮤턴트에도 12개 테스트 전부 green. look-ahead 가드가 **작동하지 않았다** | `build_evidence` 가 `decision_date` 필수 + 불일치 시 `LookAheadError` fail-closed. 러너 실경로 관통 테스트 추가. 뮤턴트 red 확인 + 원복 SHA 일치 | §2 |
| **B2** | D+20 채점불가는 단순 끝단 절단이 아니라 **정보성 검열**이고 B−A 에 더 몰림 | 검열 2종 분해 + 유계 민감도 + **주 창을 D+5 로 승격** + "동일 분포" 문언 전면 삭제 | §5.7 · §6.1 · §6.2 |
| S1 | US 라이브 가격 **몽키패치**가 재사용 표에 안 드러남 | 표에 `변조` 열 추가하고 명시 | §1.1 |
| S2 | 상수 4종이 addendum digest 밖 | addendum 안으로 이동, 이전 digest 보존 | §3 |
| S3 | 결정일 수 과대(KR 492 / US 453) | eligible **KR 442 / US 403** 로 정정, 원 grid 는 별도 표기 | §3 · §4 |
| S4 | crypto 반올림 왜곡 미정량 | 정량표 + 전 표에 `🔴비정본` 라벨 | §5.8 |
| S5 | holdout 증거를 세 시장 동일하게 서술 | 시장별 증거 표면 표로 분리 | 사용 제한 4항 |

🔴 **수리 후 네 시장 전부 재실행**했다(v3 artifact). 1차(v1)·중간(v2) 산출물은
보존한다. 결과 숫자는 나온 그대로 싣는다 — 아래 §6.2 의 판정은 수리 **전후로
바뀌지 않았고**, 바뀌었더라도 그대로 실었을 것이다.

🔴 **1차 보고 자체 정정 1건**: 테스트 통과 수를 `10 passed` 로 적었으나 해당
커밋의 실제 값은 `12 passed` 였다. 테스트 2개를 추가한 뒤 갱신 전 줄을 인용했다.
