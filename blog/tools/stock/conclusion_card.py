...
class ConclusionCard:
    ...
    def create(
        ...
        tech_signal = "매수" if rsi < 40 and macd > 0 else "매도" if rsi > 70 and macd < 0 else "중립"
        tech_color = Colors.BULLISH if tech_signal == "매수" else Colors.BEARISH if tech_signal == "매도" else Colors.NEUTRAL

        parts.append(
            InfoCard.create(
                x=x, y=y, width=card_w, height=card_h,
                title="기술적 분석", value=tech_signal,
        ...
        fund_signal = "매수" if per < 15 and roe > 10 else "매도" if per > 40 else "중립"
        fund_color = Colors.BULLISH if fund_signal == "매수" else Colors.BEARISH if fund_signal == "매도" else Colors.NEUTRAL

        parts.append(
            InfoCard.create(
                x=x + card_w + gap, y=y, width=card_w, height=card_h,
                title="펀더멘탈 분석", value=fund_signal,
        ...
        if consec >= 3:
            ...
            sd_signal = "중립"
            sd_desc = "수급 대기"
        sd_color = Colors.BULLISH if sd_signal == "매수" else Colors.BEARISH if sd_signal == "매도" else Colors.NEUTRAL

        parts.append(
            InfoCard.create(
                x=x, y=y + card_h + gap, width=card_w, height=card_h,
                title="수급 분석", value=sd_signal,
                description=sd_desc,
        ...
        overall_color = Colors.BULLISH if overall == "매수" else Colors.BEARISH if overall == "매도" else Colors.NEUTRAL

        parts.append(
            InfoCard.create(
                x=x + card_w + gap, y=y + card_h + gap, width=card_w, height=card_h,
                title="종합 판단", value=overall,
