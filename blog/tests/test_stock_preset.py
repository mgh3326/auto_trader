...
class TestStockAnalysisPresetHybridMode:
    ...
    @pytest.mark.asyncio
    async def test_generate_pngs_keeps_hybrid_svg_if_technical_png_missing(
        self, tmp_path, monkeypatch
    ) -> None:
        """Technical SVG should be kept when PNG conversion fails or is missing."""
        from blog.tools.presets.stock_analysis import StockAnalysisPreset
        from blog.tools.svg_converter import SVGConverter

        # Create a minimal valid PNG file for screenshot
        png_data = bytes([
            0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A,
            0x00, 0x00, 0x00, 0x0D, 0x49, 0x48, 0x44, 0x52,
            0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x01,
        ...
        # Mock convert_all to return paths missing the technical PNG
        async def fake_convert_all(self, files, stop_on_error=False):
            # Return only thumbnail PNG, missing technical PNG
            return [output_dir / "005930_thumbnail.png"]

        monkeypatch.setattr(SVGConverter, "convert_all", fake_convert_all)

        await preset.generate_pngs()

        # Technical SVG should still exist because technical PNG was not produced
        assert (output_dir / "005930_technical.svg").exists()

    @pytest.mark.asyncio
    async def test_generate_pngs_removes_hybrid_svg_if_technical_png_exists(
        self, tmp_path, monkeypatch
    ) -> None:
        """Technical SVG should be removed when PNG conversion succeeds."""
        from blog.tools.presets.stock_analysis import StockAnalysisPreset
        from blog.tools.svg_converter import SVGConverter
        ...
        # Pre-create the technical PNG file so exists() returns True
        technical_png = output_dir / "005930_technical.png"
        technical_png.write_bytes(png_data)

        # Mock convert_all to return all PNGs including technical
        async def fake_convert_all(self, files, stop_on_error=False):
            return [
                output_dir / "005930_thumbnail.png",
                technical_png,
                output_dir / "005930_fundamental.png",
                output_dir / "005930_supply_demand.png",
                output_dir / "005930_conclusion.png",
            ]
        ...
        # Technical SVG should be removed because technical PNG was produced
        assert not (output_dir / "005930_technical.svg").exists()
