...
### 5. Upbit 웹 트레이딩 대시보드 구축하기: CLI에서 웹으로의 진화
- **파일**: [blog_upbit_web_trading.md](blog_upbit_web_trading.md)
- **주제**: 터미널 스크립트를 웹 대시보드로 전환하여 편리한 자동매매 시스템 구축
- **내용**:
  - CLI 방식의 문제점 (터미널 3개, 경로 찾기, 외출 중 불가능)
  - 웹 대시보드 구현 (원클릭 자동매매, 실시간 프로그레스 바)
  - 기술 스택: FastAPI + Celery + Redis + localStorage
  - Celery 백그라운드 작업 및 진행 상황 추적
  - 반응형 디자인 (데스크톱/모바일)
  - 코인별 개별 작업 (분석/매수/매도)
  - localStorage로 페이지 새로고침 시 작업 복원
  - Flower 모니터링 대시보드

### Infra-1. Python 프로젝트를 Poetry에서 UV로 마이그레이션하기: 10배 빠른 의존성 관리
- **시리즈**: 개발 인프라 개선
...
- [x] **5편**: Upbit 웹 트레이딩 대시보드 구축하기: CLI에서 웹으로의 진화
- [ ] **6편**: 라즈베리파이 홈서버에 Docker 배포하기 (예정)

### 개발 인프라 시리즈 (별도 시리즈)
- [x] **Infra-1편**: Python 프로젝트를 Poetry에서 UV로 마이그레이션하기: 10배 빠른 의존성 관리
