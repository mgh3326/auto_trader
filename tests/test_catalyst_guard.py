...
def test_trim_with_positive_catalyst_flags():
    g = evaluate_catalyst_guard(
        [_ev("035420", "positive", 3)], side="trim", within_days=7
    )
...
def test_buy_with_negative_catalyst_flags():
    g = evaluate_catalyst_guard(
        [_ev("005930", "negative", 2, category="policy_regulation")],
        side="buy",
        within_days=7,
    )
...
def test_trim_with_only_negative_no_flag():
    g = evaluate_catalyst_guard(
        [_ev("005930", "negative", 2, category="lockup_expiry")],
        side="trim",
        within_days=7,
    )
...
def test_out_of_window_no_flag():
    g = evaluate_catalyst_guard(
        [_ev("035420", "positive", 30)], side="trim", within_days=7
    )
    assert g.flag is None
