...
async def test_service_groups_geopolitical_oil_into_section_and_marks_excluded(
    ...
    monkeypatch.setattr(
        news_radar_service, "get_news_articles", fake_get_news_articles
    )
    monkeypatch.setattr(
    ...
    geo_section = next(s for s in response.sections if s.section_id == "geopolitical_oil")
...
async def test_service_filters_by_risk_category(
    ...
    high = FakeArticle(
        id=1, title="Iran sanctions tighten in Hormuz",
        url="u1", market="us",
    )
    macro = FakeArticle(
        id=2, title="Fed signals rate hike",
        url="u2", market="us",
    )
    ...
    monkeypatch.setattr(news_radar_service, "get_news_readiness", fake_get_news_readiness)
    monkeypatch.setattr(
        news_radar_service,
        "format_market_news_briefing",
        fake_format_market_news_briefing,
    ...
    response = await news_radar_service.build_news_radar(
        market="us", hours=24, q=None,
        risk_category="geopolitical_oil",
        include_excluded=False, limit=50,
    )

    assert all(
        item.risk_category == "geopolitical_oil" for item in response.items
    )
...
async def test_service_returns_unavailable_readiness_when_news_pipeline_down(
    ...
    response = await news_radar_service.build_news_radar(
        market="all", hours=24, q=None, risk_category=None,
        include_excluded=True, limit=50,
    )
