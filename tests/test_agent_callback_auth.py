"""Authentication tests for the agent gateway callback endpoint."""
...
from app.routers import agent_callback
...
def client(monkeypatch: pytest.MonkeyPatch):
    ...
    app.dependency_overrides[agent_callback.get_db] = override_get_db
...
def test_agent_callback_rejects_missing_token(client: TestClient):
    res = client.post("/api/v1/agent/callback", json=_payload())
    assert res.status_code == 401


def test_agent_callback_rejects_invalid_token(client: TestClient):
    res = client.post(
...
def test_agent_callback_accepts_valid_bearer_token(client: TestClient):
    res = client.post(
        "/api/v1/agent/callback",
...
def test_agent_callback_accepts_x_agent_callback_token(client: TestClient):
    res = client.post(
        "/api/v1/agent/callback",
