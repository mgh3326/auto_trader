...
def test_lifecycle_allowance_is_exactly_void():
    """Widening this map is a deliberate act, not a side effect of the void fix.

    ``order_proposal_expire_sweep`` and ``order_proposal_redispatch`` were both
    class D in the MCP surface audit (2026-09-03) and no longer register, so the
    lifecycle bucket and its lane grant are now exactly ``order_proposal_void``.
    """
    granted = set().union(*L.LANE_PROPOSAL_LIFECYCLE_ALLOWED.values())
    assert granted == {"order_proposal_void"}
    assert L.PROPOSAL_LIFECYCLE_TOOLS == frozenset({"order_proposal_void"})
