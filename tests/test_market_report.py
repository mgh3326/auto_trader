...
# ROB-447: TestMCPGetMarketReports / TestMCPGetLatestMarketBrief / TestMCPToolRegistration
# removed — they exercised the report판 MCP wrappers (market_report_handlers), which were
# dead (shadowed by the brief판) and are deleted in this PR. The report판 SERVICE tests
# (TestGetMarketReports / TestGetLatestMarketBrief above) and the n8n write-path tests
# (below) remain. Brief판 tool registration is covered by the new real-FastMCP boot smoke
# test in tests/test_mcp_tool_registration_boot.py.
