...
class TestTechnicalIndicators:
    ...
    def test_add_indicators_function(self):
        ...
        # Check data types
        assert isinstance(result_df["macd"].iloc[0], (float, int)) or pd.isna(
            result_df["macd"].iloc[0]
        )
        assert isinstance(result_df["rsi14"].iloc[0], (float, int)) or pd.isna(
            result_df["rsi14"].iloc[0]
        )
