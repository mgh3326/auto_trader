...
def test_shipped_config_validates():
    ...
    assert policy_content_hash() == "cc9e770f3065"
...
def test_s163_parking_allowlist_adds_no_policy_key_or_value():
    ...
    assert "§S174 Toss KR parking scope 2026-09-01" in current["source"]
    assert "Scope is kis_live × equity_us only assertion" in current["source"]
...
def test_rob_1289_preserves_all_preexisting_policy_keys_and_values():
    ...
    # §176차 (2026-09-07) — three additive decision rules and NOTHING else.
...
# ---------------------------------------------------------------------------
# §176차 (2026-09-07) — buy.underwater_support_net / sell.loss_cut /
# buy.deployment_cap.
#
...
_S176_UNDERWATER_KEY = "buy.underwater_support_net"
_S176_LOSS_CUT_KEY = "sell.loss_cut"
_S176_DEPLOYMENT_CAP_KEY = "buy.deployment_cap"


def _underwater_conditions(raw):
    return raw["decision_rules"][_S176_UNDERWATER_KEY]["tiers"][0]["conditions"]


def test_s176_underwater_tier_mirrors_the_crypto_twin_with_the_sign_reversed():
    doc = TradingPolicyDocument.model_validate(_raw())
    underwater = doc.decision_rules[_S176_UNDERWATER_KEY]
...
def test_s176_underwater_size_cap_matches_the_winner_pullback_add_cap():
    """The 50% cap is §127차's, adopted rather than invented."""

    doc = TradingPolicyDocument.model_validate(_raw())
    conditions = doc.decision_rules[_S176_UNDERWATER_KEY].tiers[0].conditions
...
def test_s176_underwater_drift_fails_the_build(key, value):
    raw = _raw()
...
def test_s176_underwater_cannot_readopt_the_profitable_lot_floor():
...
@pytest.mark.parametrize("dropped", _S176_UNDERWATER_EXCLUSIONS)
def test_s176_underwater_exclusions_cannot_be_dropped(dropped):
    raw = _raw()
    rule = raw["decision_rules"][_S176_UNDERWATER_KEY]
    rule["exclusions"] = [name for name in rule["exclusions"] if name != dropped]
    with pytest.raises(ValidationError):
        TradingPolicyDocument.model_validate(raw)


def test_s176_underwater_markets_cannot_be_widened_to_crypto():
    raw = _raw()
    raw["decision_rules"][_S176_UNDERWATER_KEY]["markets"] = ["kr", "us", "crypto"]
    with pytest.raises(ValidationError):
        TradingPolicyDocument.model_validate(raw)


def test_s176_renaming_a_tier_does_not_skip_its_validators():
    """The §139차 B2 lesson, applied to all three new rules."""

    for key, tier_id in (
        (_S176_UNDERWATER_KEY, "underwater_support_net"),
        (_S176_LOSS_CUT_KEY, "loss_cut_candidate"),
        (_S176_DEPLOYMENT_CAP_KEY, "deployment_cap"),
    ):
        raw = _raw()
...
def test_s176_new_rules_reject_typo_keys():
    for key in (
        _S176_UNDERWATER_KEY,
        _S176_LOSS_CUT_KEY,
        _S176_DEPLOYMENT_CAP_KEY,
    ):
        raw = _raw()
        raw["decision_rules"][key]["bogus"] = True
        with pytest.raises(ValidationError):
...
def test_s176_loss_cut_classification_cannot_grow_an_execution_surface(key, value):
    raw = _raw()
    raw["decision_rules"][_S176_LOSS_CUT_KEY]["tiers"][0]["conditions"][key] = value
    with pytest.raises(ValidationError):
        TradingPolicyDocument.model_validate(raw)


def test_s176_loss_cut_and_underwater_cannot_both_claim_the_same_lot():
    """The deep-loss branch is gated on weak-only support by construction."""

    doc = TradingPolicyDocument.model_validate(_raw())
    loss_cut = doc.decision_rules[_S176_LOSS_CUT_KEY].tiers[0].conditions
    underwater = doc.decision_rules[_S176_UNDERWATER_KEY].tiers[0].conditions
...
def test_s176_loss_cut_leaves_the_existing_execution_keys_untouched():
...
def test_s176_deployment_cap_drift_fails_the_build(key, value):
    raw = _raw()
    raw["decision_rules"][_S176_DEPLOYMENT_CAP_KEY]["tiers"][0]["conditions"][key] = (
        value
    )
    with pytest.raises(ValidationError):
        TradingPolicyDocument.model_validate(raw)
...
def test_s176_source_records_the_three_rules_and_the_honest_limits():
    doc = TradingPolicyDocument.model_validate(_raw())

    assert "§176차 underwater averaging-down tier" in doc.source
