...
def test_ack_accepted_only_with_a_readable_order_number() -> None:
    ack = classify_order_ack(fx("ack_accepted"))
    assert (ack.state, ack.broker_order_id) == ("accepted", "1000123")


def test_success_code_without_order_number_is_uncertain_not_success() -> None:
    ack = classify_order_ack(fx("ack_success_code_without_order_number"))
    assert ack.state == "acceptance_uncertain"
    assert ack.broker_order_id is None


def test_business_rejection_is_rejected_and_message_digits_are_masked() -> None:
    ack = classify_order_ack(fx("ack_rejected_insufficient_cash"))
    assert ack.state == "rejected"
    assert ack.response_code == "40310"
    assert "12345678901" not in (ack.response_message or "")


@pytest.mark.parametrize("payload", (None, [], "text", {"Output_0": "x"}))
def test_unreadable_ack_is_uncertain(payload: object) -> None:
    assert classify_order_ack(payload).state == "acceptance_uncertain"
