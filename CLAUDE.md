...
### Binance Testnet Order Ledger (ROB-286)

`binance_testnet_order_ledger` — Binance Spot 테스트넷 주문 라이프사이클 레코드 (planned → previewed → validated → submitted → filled → tp_sl_armed → tp_sl_triggered → closed → reconciled, plus cancelled/anomaly branches).

- **ORM 모델**: `app/models/binance_testnet_order_ledger.BinanceTestnetOrderLedger`
- **서비스**: `app/services/brokers/binance/testnet/ledger/service.BinanceTestnetLedgerService` — 모든 쓰기는 이 서비스를 통해서만 허용 (11개 `record_*` 메서드)
- **리포지토리**: `app/services/brokers/binance/testnet/ledger/repository.BinanceTestnetLedgerRepository` — 서비스 내부 전용 (외부 import 금지)
- **상태 기계 검증**: `BinanceInvalidStateTransition` — 잠긴 전이 표는 service 모듈 docstring 참고
- **실행 어댑터**: `app/services/brokers/binance/testnet/execution_client.BinanceTestnetExecutionClient` — 서명된 엔드포인트 유일한 surface; 테스트넷 전용 (live 모드 없음)
- **시그너 chokepoint**: `app/services/brokers/binance/testnet/signing._sign_request_params` — 단일 HMAC-SHA256 함수
- **스캘퍼**: `app/services/scalping/{config,decision,runner,notifications}` — 결정 함수는 pure (LLM 없음, Hermes/Discord 승인 없음)
- **CLI**: `scripts/binance_testnet_scalper_smoke.py` (default-disabled, dry-run 기본), `scripts/binance_testnet_seed_instruments.py` (idempotent instrument seeder)
- **런북**: `docs/runbooks/binance-testnet-scalping.md`

**안전 경계**:
- **구조적으로 테스트넷 전용**: `BinanceTestnetExecutionClient` 클래스명, `TESTNET_HOSTS` allowlist, transport factory 모두 live 호스트를 거부함 (`BinanceLiveHostBlocked`).
- **Cross-allowlist guard**: 서명된 요청이 `PUBLIC_HOSTS` (Child B의 live allowlist) 호스트로 가면 `BinanceTestnetCrossAllowlistViolation` 발생.
- **Default-disabled**: `BINANCE_TESTNET_ENABLED` unset 시 어댑터 생성 거부 (`BinanceTestnetDisabled`).
- **Per-call operator gate**: `submit_order(...)` 는 `confirm=True` 가 매 호출마다 명시되어야 실제 HTTP 발생; default 는 `DryRunResult`.
- **스케줄러 활성화 없음**: TaskIQ/cron/Prefect 연결 없음. CLI에서만 호출. 감사 테스트(`tests/services/brokers/binance/testnet/test_audit_no_live_host.py::test_no_scheduler_activation`) 강제.
- **선물 path 없음**: `testnet.binancefuture.com` 은 `TESTNET_HOSTS` 에 없음; `reduceOnly` 파라미터 없음. 선물 follow-up 은 별도 child issue.
- **프로덕션 cutover gate**: alembic 마이그레이션은 PR에 포함되지만 operator 가 별도로 `alembic upgrade head` 실행 (deferred).
