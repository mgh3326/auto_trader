...
### 변동일 신규 0건 세션 종료 규약 (ROB-1017)

buy/discovery 세션은 종료 전에 `missed_opportunity_save`를 한 번 호출한다.
지수 당일 등락률의 절댓값이 **2%를 초과**하고 `new_buy_count=0`이면 미매수
상위 N개 후보를 순위대로 모두 전달해야 한다. 각 후보에는 `reference_price`,
`target_return_pct`, `probability`(confidence), `rejection_reason`이 필요하다.
서비스는 후보별 D+5 종가수익률 forecast와
`trigger_type=missed_opportunity` 회고를 동일 correlation으로 원자 저장한다.
정확히 ±2%인 날 또는 신규매수가 1건 이상인 날은 `required=false`이며 행을
만들지 않는다. 이 경로는 학습 DB 쓰기 전용이고 broker/order/live mutation은 없다.
