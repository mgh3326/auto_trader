...
### NHPLUG Mock Mirror Account (Stage 2)

`app/services/brokers/nhplug/` retains the bounded account discovery/read foundation and adds a mock-only KRX cash-limit mirror-order client (`orders.py`, the sole order-route owner), bounded daily execution inquiry, accepted-only NH ledgers, and scheduleless smoke/reconcile support. The mirror MCP tools are `nh_mock_*` under the default (read-gated) and `hermes-paper-kis` profiles; no scheduler is registered.
...
- **Separate mutation gate / attribution**: `NHPLUG_MOCK_ORDERS_ENABLED=true` is additionally required at each order dispatch. `dry_run=False` requires `confirm=True`; only KRX cash limit buy/sell/cancel are available. `nh_mock_place_order` requires `mirror_cohort="mock_counterfactual"`, a valid strategy/correlation, and `counterfactual_of` (the original ledger UUID); it commits `review.nh_mock_signal_ledger` before a send and writes accepted responses only through `NHMockOrderLedgerService`. `dailyOrderExecution` is the only fill authority. Non-zero broker codes are failures, never fake acceptance.
