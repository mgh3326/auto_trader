...
deploy_singletons() { local image="$1"; docker rm -f at-scheduler >/dev/null 2>&1 || true; run_scheduler "$image" >/dev/null || return 1; docker rm -f at-upbit-ws at-kis-ws >/dev/null 2>&1 || true; run_ws at-upbit-ws "$image" upbit >/dev/null || return 1; run_ws at-kis-ws "$image" kis >/dev/null || return 1; wait_ws at-upbit-ws && wait_ws at-kis-ws; }
