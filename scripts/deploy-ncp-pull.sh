...
deploy_singletons() { local image="$1" scheduler upbit kis; scheduler="$(configured_image at-scheduler 2>/dev/null || printf '%s' "$image")"; upbit="$(configured_image at-upbit-ws 2>/dev/null || printf '%s' "$image")"; kis="$(configured_image at-kis-ws 2>/dev/null || printf '%s' "$image")"; docker rm -f at-scheduler >/dev/null 2>&1 || true; if ! run_scheduler "$image" >/dev/null; then run_scheduler "$scheduler" >/dev/null || true; return 1; fi; docker rm -f at-upbit-ws at-kis-ws >/dev/null 2>&1 || true; if ! run_ws at-upbit-ws "$image" upbit >/dev/null || ! run_ws at-kis-ws "$image" kis >/dev/null || ! wait_ws at-upbit-ws || ! wait_ws at-kis-ws; then docker rm -f at-scheduler at-upbit-ws at-kis-ws >/dev/null 2>&1 || true; run_scheduler "$scheduler" >/dev/null || true; run_ws at-upbit-ws "$upbit" upbit >/dev/null || true; run_ws at-kis-ws "$kis" kis >/dev/null || true; return 1; fi; }
...
capture_mcp_state() { local i; MCP_PREVIOUS_IMAGES=(); for i in "${!MCP_PREVIOUS_NAMES[@]}"; do MCP_PREVIOUS_IMAGES[$i]="$(configured_image "at-mcp-${MCP_PREVIOUS_NAMES[$i]}" 2>/dev/null || true)"; done; }
rollback_mcp() { local fallback="$1" i name image; for i in "${!MCP_NAMES[@]}"; do name="${MCP_NAMES[$i]}"; mcp_unit_is_skipped "$name" && continue; image="${MCP_PREVIOUS_IMAGES[$((i + 2))]:-$fallback}"; [[ -n "$image" ]] || continue; docker rm -f "at-mcp-${name}" >/dev/null 2>&1 || true; run_mcp "$name" "${MCP_PORTS[$i]}" "${MCP_PROFILES[$i]}" "${MCP_TOKENS[$i]}" '' "$image" >/dev/null || return 1; done; }
deploy_mcp() { local image="$1" old new i; mkdir -p "$MCP_HEARTBEAT_DIRECTORY"; chmod 1777 "$MCP_HEARTBEAT_DIRECTORY"; capture_mcp_state; old="$(read_color mcp "$MCP_ACTIVE_COLOR_FILE" 2>/dev/null || true)"; [[ -n "$old" ]] && new="$(other_color "$old")" || new=blue; docker rm -f "at-mcp-${new}" >/dev/null 2>&1 || true; run_mcp "$new" "$(mcp_port "$new")" default MCP_AUTH_TOKEN "$new" "$image" >/dev/null || return 1; wait_mcp "$(mcp_port "$new")" || { docker rm -f "at-mcp-${new}" >/dev/null 2>&1 || true; return 1; }; for i in "${!MCP_NAMES[@]}"; do mcp_unit_is_skipped "${MCP_NAMES[$i]}" && continue; docker rm -f "at-mcp-${MCP_NAMES[$i]}" >/dev/null 2>&1 || true; run_mcp "${MCP_NAMES[$i]}" "${MCP_PORTS[$i]}" "${MCP_PROFILES[$i]}" "${MCP_TOKENS[$i]}" '' "$image" >/dev/null || { rollback_mcp "$image"; return 1; }; wait_mcp "${MCP_PORTS[$i]}" || { rollback_mcp "$image"; return 1; }; done; render_haproxy "$(read_color api "$API_ACTIVE_COLOR_FILE")" "$new" && reload_haproxy || { rollback_mcp "$image"; return 1; }; write_color "$new" "$MCP_ACTIVE_COLOR_FILE" || { rollback_mcp "$image"; return 1; }; [[ -z "$old" ]] || schedule_drain "at-mcp-${old}" "$MCP_DRAIN_SECONDS"; }
...
manual_rollback() { local previous; previous="$(read_digest "$DEPLOYED_DIGEST_PREVIOUS_FILE")" || { printf 'manual rollback digest is unavailable\n' >&2; return 1; }; docker pull "$previous"; deploy_api "$previous" && deploy_worker "$previous" && deploy_singletons "$previous" && deploy_mcp "$previous" && write_digest "$previous"; }
