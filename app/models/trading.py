from sqlalchemy import (
    BigInteger, Text, Boolean, Numeric, Enum, ForeignKey, TIMESTAMP, Interval
)
...
class UserChannel(Base):
    ...
    channel: Mapped[NotifyChannel] = mapped_column(Enum(NotifyChannel, name="notify_channel"), nullable=False)
...
class UserWatchItem(Base):
    ...
    notify_cooldown: Mapped[str] = mapped_column(Interval(), default="1 hour", nullable=False)
    is_active: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    created_at: Mapped[str] = mapped_column(TIMESTAMP(timezone=True), server_default="now()")
    updated_at: Mapped[str] = mapped_column(TIMESTAMP(timezone=True), server_default="now()", onupdate=None)