...
class ModelRateLimiter:
    ...
    async def is_model_available(self, model_name: str, api_key: str) -> bool:
        ...
        try:
            ...
            print(f"  {model_name} 모델 (API: {api_key}) 사용 제한 중. 남은 시간: {remaining_time}")
            return False
    ...
    async def set_model_rate_limit(
        ...
        try:
            ...
            await redis_client.setex(
                key,
                ttl_seconds,
                json.dumps(limit_info)
            )
            
            print(f"  {model_name} 모델 (API: {api_key}) 사용 제한 설정: {seconds}초 ({until_time.strftime('%H:%M:%S')}까지)")
    ...
    def _extract_retry_seconds(self, retry_delay: Dict[str, Any]) -> Optional[int]:
        ...
        try:
            ...
            if 'seconds' in retry_delay:
                ...
                return int(retry_delay.get('seconds', 0)) + (int(retry_delay['nanos']) // 1_000_000_000)
            else:
                # 기본 구조 확인
    ...
    async def get_model_status(self, model_name: str, api_key: str = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if api_key:
                ...
                if not rate_limit_data:
                    return {'status': 'available', 'model': model_name, 'api_key': api_key}
                ...
                for key in keys:
                    ...
                    if rate_limit_data:
                        limit_info = json.loads(rate_limit_data)
                        api_key_part = key.split(':', 2)[2]  # model_name:api_key에서 api_key 부분
                        until_time = datetime.fromisoformat(limit_info['until'])
                        remaining_time = until_time - datetime.now()
    ...
    async def clear_model_rate_limit(self, model_name: str, api_key: str = None) -> bool:
        """
        ...
        try:
            ...
            if api_key:
                ...
                for key in keys:
                    await redis_client.delete(key)
                    # retry_info 키도 함께 삭제
                    retry_key = key.replace(self._rate_limit_key_prefix, self._retry_info_key_prefix)
                    await redis_client.delete(retry_key)
                    deleted_count += 1
