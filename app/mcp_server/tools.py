...
def register_tools(mcp: FastMCP) -> None:
    ...
    async def recommend_stocks(
        ...
        """Recommend stocks by strategy with score-based budget allocation.

        Returns:
            Dictionary with recommendation payload.
            - recommendations: allocated positions with reason/score.
            - total_amount: total allocated amount.
            - remaining_budget: budget left after allocation.
            - strategy, strategy_description: resolved strategy metadata.
            - candidates_screened: number of normalized candidates before exclusions.
            - disclaimer: fixed investment disclaimer text.
            - warnings: non-fatal warnings collected during processing.

            On unexpected internal failure, returns `_error_payload(...)`:
            - error: non-empty error message.
            - source: "recommend_stocks".
            - details: traceback string for debugging.
        """
        if budget <= 0:
        ...
        try:
            ...
            if normalized_market == "kr":
                ...
                if screen_error_raw is not None:
                    ...
                    logger.warning(
                        "recommend_stocks KR screening failed: %s", screen_error
                    )
                    return {
                        "recommendations": [],
                        "total_amount": 0,
            ...
            if exclude_symbols:
                ...
                logger.debug(
                    "recommend_stocks manual exclusions=%d", len(manual_excludes)
                )
            ...
            # Strategy weights explain relative factor importance:
            # RSI(technical), valuation(PER/PBR), momentum(price change),
            # volume(liquidity), dividend(income).
