...
- `summary` fields when `include_current_price=True`: `total_buy_amount` sums `avg_buy_price * quantity` over all positions; `total_evaluation` sums `evaluation_amount`; `total_profit_loss` is derived as `total_evaluation` minus the buy amount of priced positions (positions without `evaluation_amount` are unpriced and are excluded from the P&L cost basis rather than counted as losses); `total_profit_rate` is `total_profit_loss / priced_buy_amount`. `unpriced_position_count` and `unpriced_buy_amount` disclose the unpriced remainder, so `total_profit_loss = total_evaluation - total_buy_amount + unpriced_buy_amount` always reconciles. When `include_current_price=False`, evaluation/P&L fields are `null` and all positions count as unpriced.
