...
| Kiwoom mock KR-only | `kiwoom_kr` | ROB-1159/1173 least-privilege split of `kiwoom`: default read-only/research surface plus **exactly** the eight KR `kiwoom_mock_*` tools (`kiwoom_mock_get_order_detail` included). The whole `kiwoom_mock_us_*` namespace and foreign direct broker mutations such as `kis_mock_mirror_execute_report` are physically absent; the KR registrar runs through the same allowlist proxy the restricted profiles use, and regression tests intersect the connected surface with the central direct-broker-mutation contract. Requires `MCP_AUTH_TOKEN` on network transports, and (when `KIWOOM_MOCK_ENABLED=true`) complete mock credentials plus the exact `https://mockapi.kiwoom.com` base URL at startup. |
