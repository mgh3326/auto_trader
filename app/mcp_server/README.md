...
- `get_protected_positions(account_scope=None)`
  - Read-only long-term quantity-floor heads, exact decimal declarations, revisions, and broker-confirmation evidence; no MCP declaration mutation exists.
...
- `place_order(symbol, side, order_type="limit", quantity=None, price=None, amount=None, dry_run=True, reason="", exit_reason=None, thesis=None, strategy=None, target_price=None, stop_loss=None, min_hold_days=None, notes=None, indicators_snapshot=None, defensive_trim=False, approval_issue_id=None, account_mode=None)`
  ...
  - 직접 `defensive_trim=True` 및 `exit_intent="loss_cut"`/`"cash_funding"` 경로는 fail-close하며 `order_proposal_create`를 안내한다. proposal loss-cut은 Telegram 2단계 확인 후, proposal cash-funding은 별도 폐쇄형 현금조달 증거 후에만 평균단가 1% floor를 우회할 수 있다
...
- Refreshed US positions expose `price_source`, nullable `price_asof`,
  ...
  as current.
- Live positions also expose `broker_sellable_quantity`, tactical
  `sellable_quantity`, `protected_quantity`, `protection_state`, and
  `sellable_observed`. Fresh order paths always re-read broker evidence and do
  not use this response as their send-time authority.
...
- When `TOSS_API_ENABLED=true`, Toss Open API holdings are emitted with `broker="toss"`, `source="toss_api"`. `order_routable` (and `get_cash_balance` `orderable`, `/invest` home `isTradeable`) remain gated on `TOSS_LIVE_ORDER_MUTATIONS_ENABLED` (ROB-549). Holdings and home may expose cache-aware raw and tactical sellable evidence for L1 display only. Toss live order tools revalidate sellability directly at the broker immediately before a live sell mutation. Every unprotected or off-mode live sell placement requires an explicit, finite, positive `quantity`: an orderAmount-only shape is never converted into a quantity from holdings, snapshots, or sellable caches and returns `sell_quantity_required`. In shadow/enforce observation, the same shape reaches G2 first so an active protected declaration returns `protected_quantity_unresolved`; it still never reaches POST. Successful live sell place/modify responses publish the authorizing evidence as `fresh_sellable_quantity` and `sellable_quantity_source="toss_broker_preflight"`.
- The composed KIS/Upbit/manual/Toss portfolio read model used by general holdings, home, briefing, and calendar held-key reads uses a short-lived process-shared Redis snapshot with a Redis distributed singleflight (ROB-1310). A live owner renews its lock lease; corrupt entries re-enter the same singleflight recovery path, while Redis outages/owner death retain bounded direct read-only recovery. Calendar held-key reads never fall back to full live readers: a cold/invalid snapshot returns an explicit `portfolio_snapshot_unavailable` 503 with availability metadata. The narrower Toss holdings/cash snapshot remains available for its direct broker reader; the per-symbol sellable cache is display-only, invalidated after successful sell mutations, and never a send-time sizing authority. `need_sellable=false` paths skip Toss sellable reads entirely.
- The Home-to-MCP snapshot projection preserves canonical account groups, crypto market prefixes, US native-currency P/L, and percentage-point Home ratios. It retains bounded L1 protection fields, but no snapshot field is a live order authority.
...
| Analysis readonly | `analysis_readonly` | Codex/headless read/analysis allowlist only: `get_operating_briefing`, `route_request`, `get_trading_policy`, selected quote/fundamental/analysis tools, `suggest_order_account`, `get_holdings`, `get_protected_positions`, `toss_get_positions`, the forecast/trade-journal/retrospective reads and the per-broker order-history read family (HK #657), and explicitly labeled analysis persistence. No order/cancel/modify/reconcile/preview/settings/watch/admin/manual-holdings mutation tools are registered. |
| Account read | `account_read` | TradingCodex account adapter allowlist only: existing KIS/Toss account reads, `get_protected_positions`, plus `kiwoom_mock_get_positions`, `kiwoom_mock_get_orderable_cash`, and `kiwoom_mock_get_order_history`. Kiwoom and all other mutations remain physically absent. |
