...
- The composed KIS/Upbit/manual/Toss portfolio read model used by general holdings, home, briefing, and calendar held-key reads uses a short-lived process-shared Redis snapshot with a Redis distributed singleflight (ROB-1310). Redis failures or stale/invalid entries fall back to direct read-only sources and never fabricate sellable data. The narrower Toss holdings/cash snapshot remains available for its direct broker reader; the older per-symbol sellable cache remains invalidated after successful sell mutations but is not a sizing or general-read source. `need_sellable=false` paths skip Toss sellable reads entirely.
