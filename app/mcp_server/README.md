...
- **Server-observed market evidence for every submit.** Both tools require an
  ...
  preview evidence.
- **Snapshot retrieval & build lever (ROB-913 / ROB-1165).** The following MCP tools manage market quote evidence:
  ...
  - `market_quote_snapshot_ensure(market: "kr"|"us"|"crypto", symbol: str)` ensures a fresh snapshot exists. If the latest row is fresh and submit-ready, it reuses it (`reused: true`). Otherwise, it builds a new snapshot for the symbol (`reused: false`) and validates it. Price injection is strictly forbidden; price is only fetched from the trusted server-side quote sources. Because the build path persists `market_quote_snapshots`, this tool is a DB-persistence mutation and is denied by `.claude/settings.readonly.json`; `market_quote_snapshot_latest` remains read-only.
  - `us_forecast_market_quote_snapshot_ensure(forecast_id, correlation_id)` is the `MCP_PROFILE=us-paper`-only directional-lab path. Its real FastMCP schema exposes exactly those two required strings. The server loads an open `created_by="directional-lab"` / `instrument_type="equity_us"` forecast, requires an exact persisted correlation match, derives the stored US symbol, hardcodes `market="us"`, and then reuses the canonical ensure path. Caller-supplied market, symbol, price, source, or profile is impossible. The persisted correlation is the run binding; no stronger session/run field or guaranteed forecast-target entry-mode exists in the current schema. This tool is also a denied DB-persistence mutation and is physically absent from DEFAULT, KR, crypto, and analysis-readonly profiles.
