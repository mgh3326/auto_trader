...
| Directional-lab crypto | `directional-lab-crypto` | The operator **must** launch the crypto directional-lab prompt with `MCP_PROFILE=directional-lab-crypto`. This auth-required exact allowlist contains only the prompt's crypto research/bootstrap/forecast-artifact tools plus identity-bound DB-paper account/holdings/pending reads and confirm-gated paper limit placement/reconcile/single cancel. Live Upbit, Binance Demo, Alpaca, and paper account create/reset/delete are physically absent. `paper_reconcile_orders` may write DB fills when a pending order crosses; it remains exposed for the contract path but must fail closed until separately registered in the operator allowlist. |
