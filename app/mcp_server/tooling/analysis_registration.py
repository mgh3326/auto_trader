...
def register_analysis_tools(
    ...
    @mcp.tool(
        ...
        description=(
            ...
            "for KR analyst consensus (buy/hold/sell counts + target prices, "
            "cached daily per symbol in Redis with KST-date TTL and a "
            "call-time-fresh target-upside recompute), sector labels, RSI14, and "
    ...
    @mcp.tool(
        ...
        description=(
            ...
            "KR analyst consensus (buy/hold/sell counts + target prices) is "
            "cached daily per symbol (Redis, KST-date TTL) and the displayed "
            "target-upside is recomputed each call from a fresh price so it "
            "stays intraday-current; sector labels are lazy-filled (KR Naver / "
            "US yfinance) and persisted for future calls; RSI14 is computed "
            "from the persisted snapshot closes. min_analyst_count (coverage) "
            ...
            "rate-limited) is not retried within a bounded TTL window; every "
            "skip/failure is reported under meta.enrichment_excluded (rows are "
            "never silently dropped, only the specific enrichment field is "
            "missing) and chronic failures (>=3 consecutive) are additionally "
            "surfaced under meta.chronic_failure_candidates as advisory-only — "
            "this tool never mutates kr_symbol_universe/us_symbol_universe on "
            "its own. Read-only wrt broker/order/watch state."
        ),
    )
