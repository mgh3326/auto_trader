...
def register_downside_watch_tools(mcp: FastMCP) -> None:
    mcp.tool(
        ...
        description=(
            ...
            "(idempotent — safe to rerun). A level whose current price cannot "
            "be fetched is skipped and reported separately under "
            "skipped_price_unavailable (not registered). A level already "
            "at/below its threshold IS registered — it is tagged "
            "already_breached (trigger_checklist + metadata, with the "
            "registration-time price and threshold) and may trigger on the "
            "very next scanner pass; it is not silently dropped. Call with "
            "dry_run=True first to see the already_breached count per item "
            "before committing. Notify-only registration "
