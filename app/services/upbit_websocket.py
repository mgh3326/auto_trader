...
class UpbitMyOrderWebSocket:
    ...
    def __init__(self, on_order_callback: Optional[Callable] = None, verify_ssl: bool = False):
        """
        Args:
    ...
    async def connect_and_subscribe(self, coin_pairs: Optional[list] = None):
        """
        ...
        while self.current_attempt < self.max_reconnect_attempts:
            try:
                ...
                logger.error(f"WebSocket 연결 실패 (시도 {self.current_attempt}/{self.max_reconnect_attempts}): {e}")
                
                if self.current_attempt >= self.max_reconnect_attempts:
                    logger.error("최대 재연결 시도 횟수에 도달했습니다. 연결을 중단합니다.")
                    break
                    
                logger.info(f"{self.reconnect_delay}초 후 재연결을 시도합니다...")
                await asyncio.sleep(self.reconnect_delay)
                
    async def _connect_and_subscribe_internal(self, coin_pairs: Optional[list] = None):
        """내부 연결 및 구독 로직"""
    ...
    def _create_subscribe_message(self, coin_pairs: Optional[list] = None) -> list:
        """구독 메시지 생성"""
        # 기본 메시지 구조
        message = [
            {
            ...
            async for message in self.websocket:
                try:
                    ...
                    if data.get("type") == "myOrder":
                        logger.info(f"주문/체결 데이터 수신: {data.get('code')} - {data.get('state')}")
                        ...
                        logger.info(f"다른 타입의 메시지: {data.get('type', '알 수 없음')}")
...
class UpbitOrderAnalysisService:
    ...
    def __init__(self, analyzer_callback: Optional[Callable] = None, verify_ssl: bool = False):
        """
        Args:
    ...
    async def start_monitoring(self, coin_pairs: Optional[list] = None):
        """모니터링 시작"""
        if self.is_running:
        ...
        self.websocket_client = UpbitMyOrderWebSocket(
            on_order_callback=self._handle_order_data,
    ...
    async def _handle_order_data(self, order_data: dict):
        ...
        try:
            ...
            logger.info(f"체결 감지 - {coin_name}({code}): {ask_bid} {trade_volume}개 @ {trade_price}원")
