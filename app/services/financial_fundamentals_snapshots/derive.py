...
def _growth_3y_avg(values: list[Decimal | None]) -> MetricResult:
    ...
    return MetricResult(
        value=avg, state="partial", note="fewer than 3 usable YoY deltas"
    )
...
def _gross_margin_ttm(annual: list, quarterly: list) -> MetricResult:
    ...
    if len(quarterly) >= 4:
        last4 = quarterly[-4:]
        rev = sum(
            (q.discrete_revenue for q in last4 if q.discrete_revenue is not None),
            Decimal(0),
        )
        ...
        if rev > 0 and len(cogs_vals) == 4:
            return MetricResult(
                value=(rev - sum(cogs_vals, Decimal(0))) / rev, state="ok"
            )
...
def _earnings_growth_qoq(quarterly: list) -> MetricResult:
    ...
    if g is None:
        return MetricResult(
            value=None, state="partial", note="non-positive base quarter"
        )
