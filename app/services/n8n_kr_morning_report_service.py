...
async def fetch_kr_morning_report(
    ...
    portfolio_raw = results[0] if not isinstance(results[0], Exception) else {"positions": []}
    ...
    # Process screening
    screening_data = {"total_scanned": 0, "top_n": top_n, "strategy": screen_strategy, "results": [], "summary": {}}
    if screen_task:
        try:
...
def _build_holdings(portfolio_raw: dict[str, Any]) -> dict[str, Any]:
    ...
    for pos in kr_positions:
        ...
        normalized = {
            ...
            "pnl_fmt": fmt_pnl(float(pos["profit_rate"]) * 100 if pos.get("profit_rate") is not None else None),
...
def _normalize_screening(raw: dict[str, Any], top_n: int, strategy: str | None) -> dict[str, Any]:
...
def _build_brief_text(
    ...
    sections.append("\n💼 국내주식 잔고")
    sections.append(f"KIS: {h_kis['total_eval_fmt']} ({h_kis['total_pnl_fmt']}) — {h_kis['total_count']}종목")
    sections.append(f"토스: {h_toss['total_eval_fmt']} ({h_toss['total_pnl_fmt']}) — {h_toss['total_count']}종목")
    sections.append(f"합산: {h_comb['total_eval_fmt']} ({h_comb['total_pnl_fmt']})")
    
    # Cash
    sections.append("\n💰 가용 현금")
    sections.append(f"KIS: {cash_balance['kis_krw_fmt']} | 토스: 수동 관리 | 합산: {cash_balance['total_krw_fmt']}")
