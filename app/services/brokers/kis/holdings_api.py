"""KIS API Holdings/Balance operations module.

This module provides the HoldingsAPI class that handles all balance, holdings,
and margin inquiry operations for the KIS (Korea Investment & Securities) API:
- Domestic stock holdings (주식잔고)
- Overseas stock holdings (해외주식잔고)
- Domestic cash balance (국내 예수금)
- Overseas margin (해외증거금)
- Integrated margin (통합증거금)
...
class HoldingsAPI:
    """KIS API holdings/balance operations.

    This class handles all balance, holdings, and margin inquiry operations:
    - Domestic stock holdings with pagination support
    - Overseas stock holdings with pagination support
    - Domestic cash balance inquiry
    - Overseas margin by currency
    - Integrated margin (KRW + foreign currency)
    ...
    Example:
        ...
        holdings = HoldingsAPI(transport)
        stocks = await holdings.fetch_my_stocks(is_mock=False)
    """

    def __init__(self, transport: "KISTransport") -> None:
        """Initialize the HoldingsAPI with a transport layer.
    ...
    async def fetch_my_stocks(
        ...
        if is_overseas:
            ...
            url = f"{BASE_URL}{OVERSEAS_BALANCE_URL}"
            ctx_key_fk = "CTX_AREA_FK200"
            ctx_key_nk = "CTX_AREA_NK200"
        else:
            # 국내주식 잔고조회
            tr_id = DOMESTIC_BALANCE_TR_MOCK if is_mock else DOMESTIC_BALANCE_TR
            url = f"{BASE_URL}{DOMESTIC_BALANCE_URL}"
        ...
        while page <= max_pages:
            ...
            if js.get("rt_cd") != "0":
                if js.get("msg_cd") in [
                    ...
                    # Token expired - transport handles this, but if we get here,
                    # we need to retry
    ...
    async def inquire_domestic_cash_balance(self, is_mock: bool = False) -> dict:
        ...
        js = await self._transport.request(
            "GET",
            f"{BASE_URL}{DOMESTIC_BALANCE_URL}",
            headers=hdr,
            params=params,
            timeout=5,
    ...
    async def inquire_overseas_margin(self, is_mock: bool = False) -> list[dict]:
        ...
        js = await self._transport.request(
            "GET",
            f"{BASE_URL}{OVERSEAS_MARGIN_URL}",
            headers=hdr,
            params=params,
            timeout=5,
    ...
    async def inquire_integrated_margin(
        ...
        js = await self._transport.request(
            "GET",
            f"{BASE_URL}{INTEGRATED_MARGIN_URL}",
            headers=hdr,
            params=params,
            timeout=5,
