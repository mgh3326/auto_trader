...
class OrdersAPI:
    ...
    async def inquire_overseas_orders(
        ...
        while page <= MAX_PAGES:
            ...
            logging.info(
                f"페이지 {page} 조회 (tr_cont: '{tr_cont}', NK200: "
                f"'{ctx_area_nk200[:20] if ctx_area_nk200 else 'empty'}...')"
            )

            js = await self._transport.request(
                "GET",
                f"{BASE_URL}{OVERSEAS_ORDER_INQUIRY_URL}",
                headers=hdr,
                params=params,
                timeout=10,
    ...
    async def inquire_daily_order_overseas(
        ...
        while page <= MAX_PAGES:
            ...
            js = await self._transport.request(
                "GET",
                f"{BASE_URL}{OVERSEAS_DAILY_ORDER_URL}",
                headers=hdr,
                params=params,
                timeout=10,
