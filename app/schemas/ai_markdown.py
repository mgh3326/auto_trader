...
class PortfolioMarkdownRequest(BaseModel):
    ...
    preset: PresetType = Field(
        default=PresetType.PORTFOLIO_STANCE, description="생성할 Markdown 프리셋"
    )
    include_market: str = Field(
        default="ALL", description="포함할 시장 (ALL, KR, US, CRYPTO)"
    )
...
class MarkdownResponse(BaseModel):
    ...
    metadata: dict[str, Any] = Field(
        default_factory=dict, description="추가 메타데이터 (포지션 수, 생성 시간 등)"
    )
...
class InvestmentProfile(BaseModel):
    ...
    stop_loss_philosophy: str = Field(
        default="과도한 손절보다 논리 훼손 여부를 중시", description="손절 철학"
    )
    leverage_preference: str = Field(
        default="레버리지 ETF는 축소 지향", description="레버리지 선호도"
    )
    sector_preference: str = Field(default="섹터 리더 선호", description="섹터 선호")
