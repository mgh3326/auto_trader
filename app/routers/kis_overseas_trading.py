...
async def analyze_my_overseas_stocks():
    try:
        result = await run_analysis_for_my_overseas_stocks()
...
async def execute_buy_orders():
    try:
        result = await execute_overseas_buy_orders()
        return {"success": True, **result}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e)) from e
...
async def execute_sell_orders():
    try:
        result = await execute_overseas_sell_orders()
        return {"success": True, **result}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e)) from e
...
async def run_per_stock_automation():
    ...
    result = await run_per_overseas_stock_automation()
    return {"success": True, **result}
...
async def analyze_stock(symbol: str):
    """단일 종목 분석 요청"""
    result = await analyze_overseas_stock_task(symbol)
    return {"success": True, **result}
...
async def buy_order(symbol: str):
    """단일 종목 매수 요청"""
    result = await execute_overseas_buy_order_task(symbol)
    return {"success": True, **result}
...
async def sell_order(symbol: str):
    """단일 종목 매도 요청"""
    result = await execute_overseas_sell_order_task(symbol)
    return {"success": True, **result}
